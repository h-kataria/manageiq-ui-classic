module ApplicationController::Compare
  ...
  def compare_miq(_db = nil)
    ...
    drop_breadcrumb(_("Compare %{name}") % {:name => ui_lookup(:model => @sb[:compare_db])})
  ...
  def compare_all_diff_same
    ...
    drop_breadcrumb(_("Compare %{name}") % {:name => session[:db_title]})
    @lastaction = "compare_miq"
    if params[:ppsetting]                                 # User selected new per page value
      @items_per_page = params[:ppsetting].to_i           # Set the new per page value
    end
  ...
  def drift
    ...
    drop_breadcrumb(_("'%{name}' Drift Analysis") % {:name => @drift_obj.name})
  ...
  def drift_all_same_dff
    ...
    drift_to_json(@compare)
    drop_breadcrumb(_("'%{name}' Drift Analysis") % {:name => @sb[:miq_vm_name]})
    @lastaction = "drift"
    @showtype = "drift"
  ...
  def drift_history
    ...
    drop_breadcrumb(_("Drift History"), :url => "/#{controller_name}/drift_history/#{@drift_obj.id}")
